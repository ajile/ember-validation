...
* Если требуется работать только над тестами, то можно запустить тестовый сервер отдельно: `ember test --server --launch chrome,safari,firefox,PhantomJS,opera,ie9`

### Общие положения

На данный момент мы поддерживаем Ember v1.13.7. Из Ember try удалены сценарии тестирования на релизе, канарах и бете.

Если требуется внести изменения в уже существующий функционал:

* Нужно согласовать изменения со всеми участниками проекта
* Если метод переименовывается или удалется на его месте требуется создать deprecated метод. Аналогично и для свойств.
